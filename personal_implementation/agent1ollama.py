...
class TaskDelegater(AgentExecutor):
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        # ---- ✅ TIMEOUT PROTECTION ----
        TIMEOUT_SECONDS = 10   # set the limit you want (e.g., 10 seconds)
        try:
            result_obj = await asyncio.wait_for(
                send_text_async(target_port, incoming_text),
                timeout=TIMEOUT_SECONDS
            )
            result = extract_text(result_obj) if result_obj else "No response received"
        except asyncio.TimeoutError:
            result = f"[Agent1] Downstream agent '{target_card.name}' timed out after {TIMEOUT_SECONDS} seconds."
        # --------------------------------
